namespace TextEditLib
{
	...
	{
		/// <summary>
		/// The Collapse all folds command folds all text folds (if any) such that users
		/// can get an overview on the presented text (using a top to bottom approach).
		/// </summary>
		public static readonly RoutedCommand FoldsCollapseAll = new RoutedCommand("CollapseAllFolds", null
			////, new InputGestureCollection { new KeyGesture(Key.D, ModifierKeys.Control) }
		);

		/// <summary>
		/// The Expand all folds command unfolds all text folds (if any) such that users
		/// can read all text items in a given text without having to worry about foldings.
		/// </summary>
		public static readonly RoutedCommand FoldsExpandAll = new RoutedCommand("CollapseAllFolds", null
			////, new InputGestureCollection { new KeyGesture(Key.D, ModifierKeys.Control) }
		);
	}
}
