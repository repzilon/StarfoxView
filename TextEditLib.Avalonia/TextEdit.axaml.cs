...
{
	/// <summary>
	/// Implements an AvaloniaEdit control textedit control with extensions.
	/// </summary>
	public class TextEdit : TextEditor
	{
		...
		private static readonly Color HighlightColor =
			Application.Current.PlatformSettings.GetColorValues().AccentColor1;

		private static readonly StyledProperty<Brush> EditorCurrentLineBackgroundProperty =
			AvaloniaProperty.Register<TextEdit, Brush>("EditorCurrentLineBackground",
				new SolidColorBrush(Colors.Transparent));

		public static readonly StyledProperty<Brush> EditorCurrentLineBorderProperty =
			AvaloniaProperty.Register<TextEdit, Brush>("EditorCurrentLineBorder", new SolidColorBrush(Color.FromArgb(
				0x60, HighlightColor.R, HighlightColor.G, HighlightColor.B)));

		public static readonly StyledProperty<double> EditorCurrentLineBorderThicknessProperty =
			AvaloniaProperty.Register<TextEdit, double>("EditorCurrentLineBorderThickness", 2.0);
		#endregion EditorCurrentLine Highlighting Colors

		public new static readonly StyledProperty<IHighlightingDefinition> SyntaxHighlightingProperty =
			TextEditor.SyntaxHighlightingProperty.AddOwner<TextEdit>();

		/// <summary>
		/// Document property.
		/// </summary>
		public new static readonly StyledProperty<TextDocument> DocumentProperty = TextView.DocumentProperty.AddOwner<TextEdit>();
		...
		public TextEdit()
		{
			...
			/* TODO : Command bindings (there is something in Avalonia.Labs, but its API a very different from WPF)
		...
		private static void OnSyntaxHighlightingChanged(AvaloniaPropertyChangedEventArgs<IHighlightingDefinition> obj)
		{
			var textEdit = obj.Sender as TextEdit;
			textEdit?.OnChangedFoldingInstance(obj);
		}

		private static void OnDocumentChanged(AvaloniaPropertyChangedEventArgs<TextDocument> args)
		{
			var textEdit = args.Sender as TextEdit;
			textEdit?.OnDocumentChanged(args);
		}
		...
		/// </summary>
		/// <param name="e"></param>
		private void OnChangedFoldingInstance(AvaloniaPropertyChangedEventArgs e)
		{
			try {
