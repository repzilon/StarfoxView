...
namespace Avalonia.Extensions.Controls
{
	public class PropertyViewerItem : Grid
	{
		...
		{
			Loaded += delegate { Attach(property, parentType); };
		}
		...
		{
			return Dispatcher.UIThread.Invoke(delegate {
				Children.Clear();
			...
			Dispatcher.UIThread.Invoke(delegate {
				switch (_technique) {
					...
					case TypeConversionTechniques.Custom:
						var context =
							PropertyViewer.CustomConverters[AttachedProperty.PropertyType] as
								PropertyViewer.RangeCustomConverter;
						var rangedObject = AttachedProperty.GetValue(ParentObject);
						rangedObject.GetType().GetProperty(context.ValuePropName)
							.SetValue(rangedObject, ((Slider)_inputs[0]).Value);
						break;
		...
		private void SetConvertElement()
		{
			TextBox GetTextBox(string text, bool isEnabled = true)
			{
				...
				tbox.TextChanged += delegate { ClearError(); };
				return tbox;
			}
			...
			switch (_technique) {
				case TypeConversionTechniques.DecimalConverter:
					...
					break;
				case TypeConversionTechniques.TypeConverter:
					_inputs.Add(GetTextBox(AttachedProperty.GetValue(ParentObject)?.ToString() ?? "",
						AttachedProperty.CanWrite));
					Children.Add(_inputs[0]);
					SetColumn(_inputs[0], 1);
					break;
				case TypeConversionTechniques.Custom:
					...
					var rangedObject = AttachedProperty.GetValue(ParentObject);
					var rangedType = rangedObject.GetType();
					//Next, access the property 'value'
					double setValue = (double)rangedType.GetProperty(context.ValuePropName).GetValue(rangedObject);
					double maxValue = (double)rangedType.GetProperty(context.MaxValuePropName).GetValue(rangedObject);
					double minValue = (double)rangedType.GetProperty(context.MinValuePropName).GetValue(rangedObject);
					...
					ToolTip.SetTip(slider, setValue);
					slider.ValueChanged += delegate { ToolTip.SetTip(slider, slider.Value.ToString()); };
					_inputs.Add(slider);
	...
	public partial class PropertyViewer : StackPanel
	{
		...
		internal static readonly Dictionary<Type, CustomConverter> CustomConverters =
			new Dictionary<Type, CustomConverter>();
...
#else
			return CustomConverters.TryAdd(propertyType,
				new RangeCustomConverter(valuePropertyName, maxValuePropertyName, minValuePropertyName));
#endif
		}
