...
namespace StarFox.Interop.GFX
{
	public class CAD
	{
		...
		public class CGX
		{
...
#if RENDER
			public SKBitmap Render(COL pal, int palForce = -1, int CanvasW = 128, int CanvasH = 128 * 4)
			{
				SKBitmap output = new SKBitmap(CanvasW, CanvasH);
				...
				for (int i = 0; i < (256 * 4); i++) {
					...
					SKBitmap tile = RenderTile(i, 8, palette);

					using (var g = new SKCanvas(output)) {
						g.DrawBitmap(tile, x, y,
			...
			public SKBitmap RenderTile(int tile, int size, Color[] pal, bool xflip = false, bool yflip = false)
			{
				...
				using (var g = new SKCanvas(output)) {
					var options = new SKPaint() { FilterQuality = SKFilterQuality.Low, IsAntialias = true };
					var format = GetFormat();
					for (int y = 0; y < (size / 8); y++) {
						for (int x = 0; x < (size / 8); x++) {
							int tilecalc = (tile + (!xflip ? x : ((size / 8) - x - 1)) + (!yflip ? y * 0x10 : ((size / 8) - y - 1) * 0x10)) % 0x400;
							if (format == 0) {
								g.DrawBitmap(GFX.Render.SNES.Tile2BPP(Utility.Subarray(chr, tilecalc * 16, 16), pal, xflip, yflip), 
									x * 8, y * 8, options);
							} else if (format == 1) {
								g.DrawBitmap(GFX.Render.SNES.Tile4BPP(Utility.Subarray(chr, tilecalc * 32, 32), pal, xflip, yflip), 
									x * 8, y * 8, options);
							} else {
								g.DrawBitmap(GFX.Render.SNES.Tile8BPP(Utility.Subarray(chr, tilecalc * 64, 64), pal, xflip, yflip),
									x * 8, y * 8, options);
							}
						}
					}
...
#if RENDER
			public SKBitmap Render(CGX cgx, COL col, bool allvisible = false, int Screen = -1)
			{
				//Get CGX Format
				...
				SKBitmap output = default;
				if (renderAllScreens)
					output = new SKBitmap(512 * (t / 8), 512 * (t / 8));
				else
					output = new SKBitmap(512 * (t / 8) / 2, 512 * (t / 8) / 2);
		...
		public class OBJ
		{
...
#if RENDER
			public SKBitmap Render(int seq, int frame, CGX cgx, COL col, int obj_size, int cgx_bank)
			{
			...
			public SKBitmap Render(int frame, CGX cgx, COL col, int obj_size, int cgx_bank)
			{
				//Tile Sizes
				...
				using (var g = new SKCanvas(output)) {
					g.DrawRect(0, 0, 256, 256,
						new SKPaint() { Color = new SKColor(254, 1, 254, 0), Style = SKPaintStyle.Fill });
				}
				...
				for (int i = 63; i >= 0; i--) {
					...
					SKBitmap chr = cgx.RenderTile((cgx_bank * 256) + tile, size, sprpal, xflip, yflip);
					//SKBitmap chr = RenderCGXTile((cgx_bank * 256) + tile, size, cgx, sprpal, xflip, yflip);

					using (var g = new SKCanvas(output)) {
						g.DrawBitmap(chr, (128 + x), (128 + y), new SKPaint() { IsAntialias = true, FilterQuality = SKFilterQuality.Low});
					}
				}
				return output;
			}

			public bool RenderOBJAnim(int seq, CGX cgx, COL pal, byte obj_size, byte cgx_bank, out SKBitmap[] frames, out int[] durations)
			{
