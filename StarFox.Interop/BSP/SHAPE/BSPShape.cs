...
namespace StarFox.Interop.BSP.SHAPE
{
	...
	public class BSPShape
	{
		...
		{
			get { return Header.XMax > 0 ? Header.XMax / Math.Max(1.0, LargestXPoint) : 1; }
			set { /* do nothing, only here so that fastJSON serializes it */ }
		}
		...
		{
			get { return -1 * (Header.YMax > 0 ? Header.YMax / Math.Max(1.0, LargestYPoint) : 1); }
			set { /* do nothing, only here so that fastJSON serializes it */ }
		}
		...
		{
			get { return Header.ZMax > 0 ? Header.ZMax / Math.Max(1.0, LargestZPoint) : 1; }
			set { /* do nothing, only here so that fastJSON serializes it */ }
		}
